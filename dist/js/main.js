...
;(function () {
    ...
    $('.address .save').on('click', function () {
        ...
        var tel = $('.address input').eq(1).val();
        var ad = $('.address input').eq(2).val();
        var sad = $('.address input').eq(3).val();
        arr.push({
            name: name,
            tel: tel,
            ad: ad,
        ...
        $.cookie('adds', JSON.stringify(arr));
        var adds = $.cookie('adds') ? JSON.parse($.cookie("adds")) : [];
        // console.log(adds)
        var str = '';
        // console.log(adds)
        adds.forEach(function (ind, val) {
            str = "<li class='checked'>\n                            <p>" + adds[val].name + "</p>\n                            <p>" + adds[val].tel + "</p>\n                            <p>" + adds[val].ad + adds[val].sad + "</p>\n                        </li>";
        });
        $('.box-top ul').append(str);
    });
    // if($.cookie('adds')){
    //     var str = '';
    //     var adds = $.cookie('adds') ? JSON.parse($.cookie("adds")) : [];
    //     // console.log(adds)
    //     adds.forEach((ind,val) => {
    //             str += `<li class='checked'>
    //                         <p>${adds[val].name}</p>
    //                         <p>${adds[val].tel}</p>
    //                         <p>${adds[val].ad}${adds[val].sad}</p>
    ...
    var Order = function () {
        ...
        _createClass(Order, [{
            ...
            value: function addEvent() {
                var that = this;
                $('.order-store').on("click", function (eve) {
                    var e = eve || window.event;
                    var target = e.target || e.srcElement;
                    console.log(target);
                    if (target.className == "del") {
                ...
                var onoff = 0;
                $('.box-top ul').on('click', '.checked', function () {
                    if (onoff === 0) {
                ...
                $('.goods-sum .submit').on('click', function () {
                    if (onoff === 1) {
            ...
            value: function removeCookie() {
                ...
                var i = 0;
                var onoff = this.order.some(function (val, index) {
                    // console.log(val)
                    i = index;
                    return val.id == _this4.id;
                });
                if (onoff) {
            ...
            value: function getCookie() {
                ...
                this.order = $.cookie("order") ? JSON.parse($.cookie("order")) : [];
                ...
                for (var i = 0; i < this.res.length; i++) {
                    for (var j = 0; j < this.order.length; j++) {
                        if (this.res[i].id == this.order[j].id) {
                            str += "<div class=\"goods\" index=\"" + this.order[j].id + "\">\n                                    <a class=\"goods-sa\" href=\"#\">\n                                        <div class=\"goods-img fl\">\n                                            <img class=\"small-img\" src=\"" + this.res[i].img + "\" alt=\"\">\n                                        </div>\n                                        <div class=\"goods-txt fl\">\n                                            <p>" + this.res[i].name + "</p>\n                                        </div>\n                                    </a>\n                                    <ul class=\"fr\">\n                                        <li>\uFFE5" + this.res[i].price + "</li>\n                                        <li>\n                                            <input type=\"text\" value=\"" + this.order[j].num + "\" readonly>\n                                        </li>\n                                        <li>\uFFE5" + this.res[i].price + "</li>\n                                        <li><a class=\"del\" href=\"javascript:;\">\u5220\u9664</a></li>\n                                    </ul>\n                                </div>";
                            this.sum += parseInt(this.res[i].price);
                        }
                    }
                }
                $('.sum p span i').html(this.sum);
                $('.order-store').html(str);
            }
        }]);
...
;(function () {
    var payPrice = $.cookie('sum');
    $('.money').html(payPrice);
    $('.pay-g').on('click', function () {
        if ($('.input').val() == payPrice) {
